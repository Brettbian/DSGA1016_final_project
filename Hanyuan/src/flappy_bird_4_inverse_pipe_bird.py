...
class FlappyBird(object):
    ...
    pipe_images = [rotate(load('assets/sprites/redbird-midflap.png').convert_alpha(), 180),
                   load('assets/sprites/redbird-midflap.png').convert_alpha()]
    
    pipe_images = [pygame.transform.scale(image, (int(image.get_width() * 2.0), \
                    int(image.get_height() * 2.0))) for image in pipe_images]


    bird_images = [load('assets/sprites/pipe-green.png').convert_alpha(),
                   load('assets/sprites/redbird-midflap.png').convert_alpha(),
                   load('assets/sprites/redbird-downflap.png').convert_alpha()]
    
    bird_images = [pygame.transform.scale(image, (int(image.get_width() * 0.3), \
                    int(image.get_height() * 0.1))) for image in bird_images]
    ...
    def next_frame(self, action):
        ...
        # make sure the speed going up is not too fast
        if self.current_velocity_y > self.max_velocity_y and not self.is_flapped:
            self.current_velocity_y += self.downward_speed
            # now downward speed is negative, so it will approach to self.max_velocity_y, 
